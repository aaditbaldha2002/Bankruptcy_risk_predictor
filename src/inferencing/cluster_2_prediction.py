...
@register_inferrer(2)
def cluster_2_prediction(file_path:str)->int:
    ARTIFACTS_DIR=os.path.join('artifacts','cluster_2')
    PREPROCESS_DIR=os.path.join(ARTIFACTS_DIR,'preprocessing')
    PCA_DIR=os.path.join(PREPROCESS_DIR,'pca')
    MODEL_REGISTRY=os.path.join('model_registry','latest_models','cluster_2_regressor')

    scaler=joblib.load(os.path.join(PREPROCESS_DIR,'cluster_2_standard_scaler.pkl'))
    cols_to_drop_before_pca=joblib.load(os.path.join(PREPROCESS_DIR,'cluster_2_columns_to_drop.pkl'))
    input_data = pd.read_csv(file_path)
    dropped_cols=joblib.load(os.path.join(PCA_DIR,'cluster_2_columns_to_drop.pkl'))
    pca_pairs_df=joblib.load(os.path.join(PCA_DIR,'cluster_2_pca_pairs_used.pkl'))
    pca_models=joblib.load(os.path.join(PCA_DIR,'cluster_2_fitted_pca_models.pkl'))
    ...
    cols_to_drop_after_pca=joblib.load(os.path.join(PREPROCESS_DIR,'cluster_2_cols_to_retain_after_pca.pkl'))
